...
./launch-run.sh -n 1,2,4,8 -s 30000      # sequential worker-node sweep
./launch-run.sh --cpu -n 2 -s 100        # CPU
./launch-run.sh --cpu --dedicated-coordinator -n 13 -s 30000
                                            # 13 workers + 1 coordinator
```
...
runs. `latest_result_dir.txt` remains a one-line pointer to the newest submitted
job-specific directory; resolve that small pointer once before a copy rather
than copying a stale legacy `result_dir/` or following a mutable symlink.
`latest_result_dirs.tsv` describes every entry in the latest invocation,
including worker and allocated node counts, job/result IDs, final Slurm state,
and node counts skipped by a sweep. Each submitted `result_dir_<jobid>/` also
contains `launch_metadata.tsv`. See `./launch-run.sh --help` for the full flag
list (queries filter, output path, GDS toggle, profiling, metrics, …).

`-n` accepts one positive worker-node count or a comma-separated sweep such as
`-n 1,2,4,8`. Sweep entries run strictly in the requested order because they
share generated configuration and live-log paths. Before each entry, the
launcher counts currently IDLE nodes in the selected Slurm partition (and the
explicit `NODELIST`, when set). An entry with insufficient immediately idle
capacity is skipped and recorded in the manifest; sweeps never prompt or leave
such entries queued. The check is a point-in-time estimate—additional raw
`sbatch` constraints and scheduler priority can still affect eligibility.
Use `-n` and `-g` for node/worker resources; conflicting raw Slurm
`--nodes`/`-N`, `--gres`, and `--gpus`/`-G` overrides are rejected so the
preflight and recorded allocation cannot diverge from the submitted job.
Every sweep entry receives the same exported environment as the launcher via
Slurm's `--export=ALL`; inline assignments immediately before
`./launch-run.sh` are exported as well. Node-dependent launcher values are the
only values recomputed for each entry.

For a single-node-count launch, insufficient idle capacity produces an
interactive prompt asking whether to submit and wait in Slurm. The default is
no, and a non-interactive launcher will not submit without confirmation. If a
queued job is accepted, the launcher prints its current Slurm state and pending
reason while waiting. Pressing Ctrl-C while a job is live asks whether to
`scancel` it before the launcher exits; declining leaves it running and prints
the explicit cancellation command. A node-count sweep retains the existing
90-second UCX-port/Slurm-cleanup cooldown once after each submitted job and
before the next sweep availability check; set
`SWEEP_INTER_RUN_SLEEP=0` only when the environment guarantees clean port
release.

`--dedicated-coordinator` keeps `-n` as the number of worker hosts and asks
Slurm for one additional host. With no `NODELIST` override, Slurm chooses the
entire allocation automatically: the first expanded hostname runs only the
Java coordinator and the remaining `-n` hosts run workers. If `NODELIST` is
set, include `-n + 1` eligible hosts; its first expanded hostname is reserved
for the coordinator. This mode does not place anything on the login/head node.
...
worker host and ignore accelerator/HBM-only NUMA IDs. Local worker slots are
