...
function start_perf_sampler {
    ...
    for ((index = 0; index < ${#WORKER_SRUN_IDS[@]}; ++index)); do
        if [[ "${WORKER_SRUN_IDS[${index}]}" == "${worker_id}" ]]; then
            worker_node="${WORKER_SRUN_NODES[${index}]}"
            break
        fi
    done
    [[ -n "${worker_node}" ]] || \
        echo_error "Could not locate worker ${worker_id} in the active worker steps"
    ...
    echo "Starting host perf sidecar for worker ${worker_id} on ${worker_node}..."
    srun -N1 -w "${worker_node}" --ntasks=1 --overlap --export=ALL \
        bash "${sampler}" \
            --worker-id "${worker_id}" \
            --control-dir "${LOGS}" \
...
function prepare_cpu_numa_layout {
    ...
    if [[ "${LEGACY_CPU_NUMA:-0}" == "1" ]]; then
        echo "WARNING: reproducing legacy CPU NUMA behavior: node-bound execution with full-host worker sizing."
    fi
...
EOS
        ...
        if [[ -z "${reference_signature}" ]]; then
            ...
            echo_error "CPU NUMA topology on ${node} differs from the reference worker host; heterogeneous CPU resource tuning is not supported"
        fi
...
function preflight_cluster_ports {
    ...
    CLUSTER_PORT_SPECS_BY_NODE["${COORD}"]="coordinator:${PORT}"
    for node in ${WORKER_NODE_NAMES}; do
    ...
    (( worker_id == NUM_WORKERS )) || \
        echo_error "Port preflight mapped ${worker_id} workers; expected ${NUM_WORKERS}"

    for node in ${ALLOCATED_NODE_NAMES}; do
        specs="${CLUSTER_PORT_SPECS_BY_NODE["${node}"]:-}"
        [[ -n "${specs}" ]] || echo_error "No coordinator or worker ports assigned to ${node}"
...
# local worker slots on every host in $WORKER_NODE_NAMES, then block until all
...
function start_cluster {
    ...
    (( worker_id == NUM_WORKERS )) || \
        echo_error "Started ${worker_id} workers; expected ${NUM_WORKERS}"
...
function inject_benchmark_metadata {
    ...
    python3 - "${result_file}" \
        ...
        "${WORKER_IMAGE}" "${image_digest}" "${engine}" \
        "${LEGACY_CPU_NUMA:-0}" "${DEDICATED_COORDINATOR:-0}" \
        "${NUM_ALLOCATED_NODES:-${NUM_NODES}}" <<'PY'
import json
import os
import pathlib
import sys
...
context.update(
    {
        ...
        "engine": engine,
        "legacy_cpu_numa": legacy_cpu_numa == "1",
        "dedicated_coordinator": dedicated_coordinator == "1",
        "allocated_node_count": int(allocated_node_count),
    }
)
