...
if [[ "${VARIANT_TYPE}" == "cpu" ]]; then
  ...
  if [[ ! "${LEGACY_CPU_NUMA:-0}" =~ ^[01]$ ]]; then
    echo_error "ERROR: LEGACY_CPU_NUMA must be 0 or 1; got '${LEGACY_CPU_NUMA:-}'."
  fi
  if [[ "${LEGACY_CPU_NUMA:-0}" == "1" ]]; then
    [[ "${USE_NUMA:-0}" == "1" ]] || \
      echo_error "ERROR: LEGACY_CPU_NUMA=1 requires USE_NUMA=1."
    (( CPU_WORKERS_PER_HOST == 1 )) || \
      echo_error "ERROR: LEGACY_CPU_NUMA=1 is intentionally limited to one CPU worker per host."
  fi
  ...
  if [[ "${USE_NUMA:-0}" == "1" ]]; then
    ...
    if [[ "${LEGACY_CPU_NUMA:-0}" == "1" ]]; then
      CPU_RESOURCE_POLICY="legacy-node-bound-host-sized"
      echo_warning "WARNING: LEGACY_CPU_NUMA=1 keeps full-host CPU/RAM sizing while binding the worker to ${CPU_NUMA_WORKER_LAYOUT}."
    else
