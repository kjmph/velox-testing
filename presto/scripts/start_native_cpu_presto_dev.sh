...
Draft CPU developer start script. It keeps orchestration Python dependencies in
...
DEV_OPTIONS:
    --restart-target all|coordinator|worker|none
        ...
        "all" is bootstrap/recovery. Use "worker" after native worker code
    ...
    --dev-presto-dependencies
        ...
        presto-verifier.
    --presto-source PATH
        Build the coordinator and native CPU workers from this Presto source
...
START_OPTIONS:
    ...
    -b/--build, -w/--num-workers, --wait, --no-cache, --build-type,
...
function apply_dev_node_addresses() {
  ...
  for i in "${!worker_services[@]}"; do
    local worker_node_config
    if [[ "${#worker_services[@]}" -eq 1 ]]; then
      worker_node_config="${config_dir}/etc_worker/node.properties"
    else
      worker_node_config="${config_dir}/etc_worker_${i}/node.properties"
    fi
    [[ -f "$worker_node_config" ]] || continue
    set_properties_file_value "node.internal-address" "${worker_services[$i]}" "$worker_node_config"
  done
}

function cpu_worker_services() {
  if [[ -f "$TEMPLATE_PATH" && -n "$NUM_WORKERS" && "$NUM_WORKERS" -gt 1 && "$SINGLE_CONTAINER" == "false" ]]; then
    local i
    for ((i = 0; i < NUM_WORKERS; i++)); do
      echo "presto-native-worker-cpu-${i}"
    done
  else
    echo "presto-native-worker-cpu"
  fi
}
...
function opposite_variant_worker_containers() {
  active_presto_worker_containers | grep -E '^presto-native-worker-gpu(-[0-9]+)?$|^presto-java-worker$' || true
}
...
function render_dev_network_override() {
  ...
  local override_path="${rendered_dir}/docker-compose.cpu-dev-network.yml"
  local worker_services=()
  local i
...
function render_dev_sccache_override() {
  local rendered_dir=$1
  local override_path="${rendered_dir}/docker-compose.cpu-dev-sccache.yml"
  local worker_services=()
  local sccache_auth_dir="${SCCACHE_AUTH_DIR:-${HOME}/.sccache-auth}"
  local service
  mkdir -p "$rendered_dir"
  mapfile -t worker_services < <(cpu_worker_services)

  {
    ...
    for service in "${worker_services[@]}"; do
      printf "  %s:\n" "$service"
      printf "    build:\n"
      printf "      ulimits: {nofile: {soft: 500000, hard: 500000}}\n"
      printf "      secrets:\n"
      printf "        - github_token\n"
    ...
    printf "\nsecrets:\n"
    printf "  github_token:\n"
    printf "    file: \"%s/github_token\"\n" "$sccache_auth_dir"
    printf "  aws_credentials:\n"
    printf "    file: \"%s/aws_credentials\"\n" "$sccache_auth_dir"
  } > "$override_path"
...
function render_dev_coordinator_override() {
  local rendered_dir=$1
  local override_path="${rendered_dir}/docker-compose.cpu-dev-coordinator.yml"
...
function render_dev_native_source_dockerfile() {
  local rendered_dir=$1
  local dockerfile_path="${rendered_dir}/native_build.cpu-dev-sources.dockerfile"
  ...
  relative_velox_source="$(source_path_in_build_context "$(effective_velox_source)" "--velox-source")"
...
function render_dev_native_source_override() {
  local rendered_dir=$1
  local override_path="${rendered_dir}/docker-compose.cpu-dev-native-sources.yml"
  local dockerfile_path
...
function build_targets_include_cpu_worker() {
  local worker_services=()
  local service
  mapfile -t worker_services < <(cpu_worker_services)
  for service in "${worker_services[@]}"; do
...
if build_targets_include_cpu_worker && [[ -n "$DEV_PRESTO_SOURCE" || -n "$DEV_VELOX_SOURCE" ]]; then
...
function container_ip_on_project_network() {
  local container=$1
...
for line in os.environ["EXPECTED_WORKER_ENDPOINTS"].splitlines():
    service, ip, port = line.split()
    alternatives = (f"http://{ip}:{port}", f"http://{service}:{port}")
    expected.append((service, alternatives))
...
if not quiet:
    print("Coordinator worker endpoints match current containers.")
PY
}
...
PY
    ...
    if [[ "$workers" -ge "$NUM_WORKERS" ]]; then
      if verify_cpu_worker_endpoints true; then
        endpoints_ready=true
        break
      fi
      echo "  worker count is sufficient, but coordinator still has stale worker endpoints"
    fi
    sleep 5
  done
